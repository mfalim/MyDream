        ...
        <div class="col-lg-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">Tambahkan event client atau event organizer internal</p>
                </div>
                <div>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <div>
                                <h5 class="fw-bold mb-0">Tipe Event</h5>
                                <small class="text-muted">Pilih apakah ini event client atau event organizer</small>
                            </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <select class="form-select" name="event_category" id="eventCategorySelect" required>
                                    <option value="">Pilih Jenis Event</option>
                                    <option value="client">Event Client (Wedding dari Booking)</option>
                                    <option value="organizer">Event Organizer (Technical Meeting, dll)</option>
                                </select>
                                <small class="text-muted">Event client terkait dengan booking, event organizer untuk keperluan internal tim</small>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Section 1: Pilih Booking (untuk client event) -->
                <div class="card border-0 shadow-sm mb-4" id="bookingSection" style="display: none;">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <div>
                                ...
                                <small class="text-muted">Pilih booking yang sudah tersedia</small>
                            </div>
                            <span class="badge bg-danger ms-auto">WAJIB</span>
                        </div>
                                ...
                                <select class="form-select" name="booking_id" id="bookingSelect">
                                    ...
                                    @foreach($bookings as $booking)
                                        <option value="{{ $booking->id }}" 
                                            ...
                                            {{ $booking->client->groom_name }} & {{ $booking->client->bride_name }} - {{ $booking->venue_name }} ({{ $booking->status }})
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="card border-0 shadow-sm mb-4" id="eventDaySection" style="display: none;">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <div>
                                <h5 class="fw-bold mb-0">Pilih Event Day</h5>
                                <small class="text-muted">Pilih hari acara atau buat hari baru</small>
                            </div>
                            <span class="badge bg-danger ms-auto">WAJIB</span>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label fw-semibold">Event Day <span class="text-danger">*</span></label>
                                <select class="form-select" name="event_day_id" id="eventDaySelect">
                                    <option value="">Pilih Event Day atau Buat Baru</option>
                                    <option value="new">+ Buat Event Day Baru</option>
                                </select>
                                <small class="text-muted">Event Day mengelompokkan event berdasarkan hari</small>
                            </div>

                            <!-- New Event Day Form -->
                            <div id="newEventDayForm" style="display: none;">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">Nama Event Day</label>
                                        <input type="text" class="form-control" name="event_day_name" placeholder="Contoh: Hari Pertama / Main Event">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">Tanggal</label>
                                        <input type="date" class="form-control" name="event_day_date">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card border-0 shadow-sm mb-4" id="teamSection" style="display: none;">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-warning" id="addTeamMemberBtn">+ Tambah Anggota Tim</button>
                        </div>
                        ...
                        <div class="team-list" id="teamList">
                            <div class="row g-3 mb-3 team-item">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select team-select" name="team_member_id[]">
                                        <option value="">Pilih Member</option>
                                        @foreach($members as $member)
                                            <option value="{{ $member->id }}">
                                                {{ $member->name }} - {{ $member->position ?? $member->division }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-5">
                                    ...
                                    <input type="text" class="form-control" name="team_member_role[]" placeholder="Contoh: Lead Coordinator">
                                </div>
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <small class="text-muted">Pilih paket atau vendor custom untuk event ini</small>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12">
                                <label class="form-label fw-semibold">Pilih Paket Vendor</label>
                                ...
                                <small class="text-muted">Jika memilih paket, vendor akan otomatis dipilih dari paket</small>
                            </div>
                        </div>

                        <!-- Package Vendors Display -->
                        ...
                        <div id="customVendorSection">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <div>
                                    ...
                                    <small class="text-muted d-block">Pilih vendor yang dibutuhkan untuk event ini</small>
                                </div>
                            ...
                            <div class="vendor-list" id="vendorList">
                                <div class="row g-3 mb-3 vendor-item">
                                    <div class="col-md-5">
                                        ...
                                        <select class="form-select vendor-select" name="vendor_id[]">
                                    ...
                                    <div class="col-md-2">
                                        <label class="form-label fw-semibold">Waktu Mulai</label>
                                        <input type="time" class="form-control" name="vendor_start_time[]" placeholder="08:00">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="form-label fw-semibold">Waktu Selesai</label>
                                        <input type="time" class="form-control" name="vendor_end_time[]" placeholder="17:00">
                                    </div>
                                    <div class="col-md-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const eventDaySection = document.getElementById('eventDaySection');
    ...
    const newEventDayForm = document.getElementById('newEventDayForm');
    ...
    eventCategorySelect.addEventListener('change', function() {
        if (this.value === 'client') {
            ...
            // For organizer events, redirect to organizer event form
            window.location.href = '/admin/organizer-event/create';
        } else {
    ...
    bookingSelect.addEventListener('change', function() {
        ...
        if (this.value) {
            ...
            eventDaySection.style.display = 'block';
            
            fetch(`/admin/booking/${this.value}/event-days`)
                .then(response => response.json())
                .then(eventDays => {
                    eventDaySelect.innerHTML = '<option value="">Pilih Event Day atau Buat Baru</option><option value="new">+ Buat Event Day Baru</option>';
                    
                    eventDays.forEach(eventDay => {
                        const option = document.createElement('option');
                        option.value = eventDay.id;
                        option.textContent = `${eventDay.name} - ${eventDay.event_date}`;
    ...
    eventDaySelect.addEventListener('change', function() {
        if (this.value === 'new') {
            newEventDayForm.style.display = 'block';
            eventDetailSection.style.display = 'block';
            teamSection.style.display = 'block';
            vendorSection.style.display = 'block';
        } else if (this.value) {
            newEventDayForm.style.display = 'none';
            eventDetailSection.style.display = 'block';
            teamSection.style.display = 'block';
            vendorSection.style.display = 'block';
        } else {
    ...
    function attachRemoveEvent(btn) {
        btn.addEventListener('click', function() {
            ...
            if (vendorItems.length > 1) {
                ...
                alert('Minimal harus ada satu slot vendor!');
            }
        });
    }
    ...
    function attachRemoveTeamEvent(btn) {
        btn.addEventListener('click', function() {
            ...
            if (teamItems.length > 1) {
                ...
                alert('Minimal harus ada satu slot tim!');
            }
        });
    }
